...
15. Capture task overrides select variants only. Production normally follows the registered attempt ladder. The player can explicitly override Codex tasks with Astra in Settings; otherwise Astra is never selected automatically. The constrained OpenAI evaluation-primary mode is available only while capture is enabled.
